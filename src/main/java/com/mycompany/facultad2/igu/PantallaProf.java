...
public class PantallaProf extends javax.swing.JFrame {
    ...
    private void cargarTabla() {
        ...
        for(Profesor p: profesores) {
            if( p.getNombreProf().equals(usuario.getUsuario())) {
                pr = p;
            }
        }
        ...
        if( calificaciones != null ) {
            ...
            for(Curso cu: pr.getCursosP()) {
                for(Calificacion c: cu.getCalificaciones()) {
                    Object[] objeto = {cu.getId(), cu.getNombre(), 
                        c.getUnAlumno().getId(), c.getUnAlumno().getNombre(), c.getNota()};
