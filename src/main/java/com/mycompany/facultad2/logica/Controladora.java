...
public class Controladora {
    ...
    public void guardarProfesor(String usuario) {
        List<Curso> cursosP = null;
        
        Profesor prof = new Profesor();
        prof.setNombreProf(usuario);
        prof.setCursosP(cursosP);
    ...
    public List<Profesor> traerProfesores() {
        return controlPersis.traerProfesores();
    }

    public void agregarProfesor(int id_curso, String prof) {
        Curso curso = controlPersis.traerCurso(id_curso);
        Profesor pr = null;
        
        List<Profesor> profesores = controlPersis.traerProfesores();
        for(Profesor p: profesores) {
            if( p.getNombreProf().equals(prof) ) {
                pr = p;
            }
        }
        
        if( pr != null && curso != null) {
            List<Profesor> profes = curso.getProfesores();
            profes.add(pr);
            List<Curso> cursos = pr.getCursosP();
            cursos.add(curso);

            pr.setCursosP(cursos);
            curso.setProfesores(profes);

            controlPersis.agregarProfesor(pr,curso);
        } 
        
    }

    public void sacarProfesor(int id_curso, String prof) {
        Curso curso = controlPersis.traerCurso(id_curso);
        Profesor pr = null;
        ...
        if( pr != null && curso != null) {
            ...
            eliminarCursoProfesor(cursos, profes, curso, pr);

            pr.setCursosP(cursos);
            curso.setProfesores(profes);
            
            controlPersis.sacarProfesor(pr,curso);
        } 
        
    }

    private void eliminarCursoProfesor(List<Curso> cursos, List<Profesor> profes, 
            Curso curso, Profesor pr) {
        for(int i=0; i<cursos.size(); i++) {
            if( cursos.get(i).getId() == curso.getId() ) {
                cursos.remove(i);
            }
        ...
        for(int i=0; i<profes.size(); i++) {
            if( profes.get(i).getIdProf() == pr.getIdProf() ) {
                profes.remove(i);
            }
        }
    }
