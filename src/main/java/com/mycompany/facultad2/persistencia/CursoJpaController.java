...
public class CursoJpaController implements Serializable {
    ...
    public void create(Curso curso) {
        ...
        if (curso.getProfesores() == null) {
            curso.setProfesores(new ArrayList<Profesor>());
        }
        ...
        try {
            ...
            List<Profesor> attachedProfesores = new ArrayList<Profesor>();
            for (Profesor profesoresProfesorToAttach : curso.getProfesores()) {
                profesoresProfesorToAttach = em.getReference(profesoresProfesorToAttach.getClass(), profesoresProfesorToAttach.getIdProf());
                attachedProfesores.add(profesoresProfesorToAttach);
            }
            curso.setProfesores(attachedProfesores);
            em.persist(curso);
            ...
            for (Profesor profesoresProfesor : curso.getProfesores()) {
                profesoresProfesor.getCursosP().add(curso);
                profesoresProfesor = em.merge(profesoresProfesor);
            }
    ...
    public void edit(Curso curso) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<Profesor> profesoresOld = persistentCurso.getProfesores();
            List<Profesor> profesoresNew = curso.getProfesores();
            ...
            List<Profesor> attachedProfesoresNew = new ArrayList<Profesor>();
            for (Profesor profesoresNewProfesorToAttach : profesoresNew) {
                profesoresNewProfesorToAttach = em.getReference(profesoresNewProfesorToAttach.getClass(), profesoresNewProfesorToAttach.getIdProf());
                attachedProfesoresNew.add(profesoresNewProfesorToAttach);
            }
            profesoresNew = attachedProfesoresNew;
            curso.setProfesores(profesoresNew);
            ...
            for (Profesor profesoresOldProfesor : profesoresOld) {
                if (!profesoresNew.contains(profesoresOldProfesor)) {
                    profesoresOldProfesor.getCursosP().remove(curso);
                    profesoresOldProfesor = em.merge(profesoresOldProfesor);
                }
            }
            for (Profesor profesoresNewProfesor : profesoresNew) {
                if (!profesoresOld.contains(profesoresNewProfesor)) {
                    profesoresNewProfesor.getCursosP().add(curso);
                    profesoresNewProfesor = em.merge(profesoresNewProfesor);
                }
            }
            em.getTransaction().commit();
        } catch (Exception ex) {
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            for (Profesor profesoresProfesor : profesores) {
                profesoresProfesor.getCursosP().remove(curso);
                profesoresProfesor = em.merge(profesoresProfesor);
            }
            em.remove(curso);
            em.getTransaction().commit();
